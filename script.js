...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* --------------------------------------------------------------------------
     4. Creator Image Lightbox
     -------------------------------------------------------------------------- */
  const creatorImage = document.getElementById('creatorImage');
  const creatorLightbox = document.getElementById('creatorLightbox');
  const creatorLightboxClose = document.getElementById('creatorLightboxClose');

  const openCreatorLightbox = () => {
    if (!creatorLightbox) return;
    creatorLightbox.classList.add('active');
    creatorLightbox.setAttribute('aria-hidden', 'false');
    document.body.style.overflow = 'hidden';
  };

  const closeCreatorLightbox = () => {
    if (!creatorLightbox) return;
  ...
  if (creatorImage) {
    creatorImage.addEventListener('click', openCreatorLightbox);
    creatorImage.addEventListener('keydown', (e) => {
  ...
  if (creatorLightboxClose) {
    creatorLightboxClose.addEventListener('click', closeCreatorLightbox);
  }

  if (creatorLightbox) {
    creatorLightbox.addEventListener('click', (e) => {
      if (e.target === creatorLightbox) closeCreatorLightbox();
    });
  }

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && creatorLightbox?.classList.contains('active')) {
